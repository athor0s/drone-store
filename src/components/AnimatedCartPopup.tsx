...
export default function AnimatedCartPopup() {
  const { state, removeFromCart, updateQuantity, getCartTotal } = useCart();
  const { showCartPopup, animationOrigin, hideCartPopup } = useCartAnimation();
  const [isVisible, setIsVisible] = useState(false);
  const [scale, setScale] = useState(0);

  useEffect(() => {
    if (showCartPopup && animationOrigin) {
      setIsVisible(true);
  ...
  }, [showCartPopup, animationOrigin]);

  if (!isVisible || !animationOrigin) return null;
  ...
  return (
    <>
      ...
      >
        <div 
          className="absolute w-96 bg-white/95 backdrop-blur-md border border-gray-200 rounded-[2rem] shadow-2xl pointer-events-auto"
          style={{
            ...popupStyle,
            top: `${animationOrigin.y + 20}px`,
            left: `${animationOrigin.x - 384}px`, // 384px = w-96
          }}
        >
          <div className="p-6">
